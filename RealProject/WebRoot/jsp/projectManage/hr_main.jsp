  ...
  <head>
	...
	<script type="text/javascript">
		...
		function delAllStaffs(){
			...
			var ids = document.getElementsByName("ids");
			var gnl=confirm("确定将以下员工删除出项目组吗？删除后，这些员工将无权查看项目组中全部文件,并且默认无法删除已被分配任务的员工和项目经理");  
			if (gnl==true){ 
				var str="";
			for(var i=0;i<ids.length;i++)
			{
    	...
    	<div class="row">
    		...
    		<div class="col-lg-8 xumode">
   	        <div class="panel panel-primary">
				...
				<!-- 权限认证 -->
				<c:if test="${not empty deleteAndAddStaff }">
					<button type="submit" class="btn btn-primary" style="float: right;" data-toggle="modal" onclick="delAllStaffs()" >删除</button><!--  onclick="delAllStaffs()"-->
				</c:if>
                <br/><br/>
                </div>
            </form>		
...
<div class="modal fade" id="handupCc" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
        <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="delAllStaffs()" >确定</button>
      </div>
    </div>
  </div>
</div>
