		...
		try {
			{
				...
				try {
					ls = StaffDao.findallstageleader(pno);
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				List<Staff> sl = new ArrayList<Staff>();
				try {
					sl = StaffDao.findalltaskleader(pno);
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				...
				{
					System.out.println(sl.get(i).getStaffno());
				}
				...
				for(int i=0;i<staffnos.length;i++)
				{
					...
					{
						if(staffnos[i].equals(sl.get(k).getStaffno()))
						{
							flag=1;
						}
					}
					if(flag==0){
						System.out.println("debug2+"+staffnos[i]);
						a.add(staffnos[i]);
					}
				...
				{
					System.out.println("debug:"+a.get(j));
				}
				
				String[] bStrings = new String[a.size()];
				...
				try{
					...
					StaffDao.delAllStaffs(bStrings, pno);
					for(int i=0;i<bStrings.length;i++)
					{
						ifd.insertInformhr(pno,"S1", bStrings[i],me);
					}
				}catch(SQLException e){
					e.printStackTrace();
				}finally{
